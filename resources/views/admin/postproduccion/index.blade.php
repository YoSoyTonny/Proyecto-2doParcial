    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <a href="{{route('postproduccion.create')}}" class="btn btn-primary">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            @foreach($postproducciones ?? '' as $postproduccion)
                                <tr>
                                    <td class="titulo_noticia">{{$postproduccion->categoria}}</td>
                                    <td>{{$postproduccion->tema}}</td>
                                    <td>{{$postproduccion->estado}}</td>

                                    <td>
                                    <form method="POST" action="{{route('postproduccion.index', $postproduccion->id)}}">
                                                        <a href="{{route('postproduccion.show',$postproduccion->id)}}" class="btn btn-primary">
                                                            <i class="fas fa-eye"></i>
                                                        </a>

                                                        <a href="{{route('postproduccion.edit',$postproduccion->id)}}" class="btn btn-success">
                                                            <i class="fas fa-edit"></i>
                                                        </a>
                                                        </a>
                                                        @csrf
                                            @method('DELETE')
                                            <button type="button" id_noticia="{{$postproduccion->id}}"
...
<script>
    function doClickEliminar(e) {
        ...
        $("#formEliminar").attr('action','/postproduccion/' + idNoticia);
